...
export function parseInline(raw: string): InlineSegment[] {
  ...
  while (cursor < raw.length) {
    ...
    if (nextDelim.kind === "code") {
      ...
      // Flatten: strip inner formatting markers so `==**foo**==` doesn't
      // leak raw `**` into the output. We keep only the text content,
      // letting the outer delimiter's style (bold/highlight) win.
      const flattened = nested.map((s) => s.content).join("");
      segments.push({ kind: nextDelim.kind, content: flattened });
    }
