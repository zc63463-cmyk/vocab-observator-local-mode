...
function getEntryHref(entry: VocabGraphEntry, basePath = "/words/") {
  const slug = firstText(entry.slug);
...
function createNodeFromCandidate(
  ...
  return {
    href: targetEntry ? getEntryHref(targetEntry, basePath) : undefined,
...
/**
 * Cached variant of `buildLocalVocabGraph`. The output is tiny (a few-dozen
 * nodes/edges) so it sits well inside Next.js's 2 MB `unstable_cache` limit.
 * This avoids re-materialising the full 5 000-row word index on every page
 * request just to build the local topology graph.
 */
export const getCachedVocabGraphForSlug = unstable_cache(
  async (slug: string, basePath = "/words/"): Promise<VocabGraphData | null> => {
    const [result, allEntries] = await Promise.all([
      getPublicWordBySlug(slug),
      getAllPublicWordIndexEntries(),
    ]);
    if (!result.word) return null;
    return buildLocalVocabGraph(result.word, allEntries, basePath);
  },
  ["vocab-graph-for-slug"],
  { revalidate: 86400 },
);
...
export function buildLocalVocabGraph(
  ...
  const centerNode: VocabGraphNode = {
    href: getEntryHref(entry, basePath),
    id: centerId,
  ...
  for (const candidate of candidates) {
    ...
    const targetNode = createNodeFromCandidate(candidate, targetEntry, basePath);
