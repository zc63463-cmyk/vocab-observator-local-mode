...
async function runSnapshot(request: NextRequest) {
  ...
  const totalPublic = await countWhere(admin, (q) => q.eq("is_published", true));
  const totalActive = await countWhere(admin, (q) =>
    q.eq("is_published", true).eq("is_deleted", false),
  );
  const totalDeleted = await countWhere(admin, (q) => q.eq("is_deleted", true));
  ...
  for (const prefix of env.wordsPrefixes) {
    const rows = await countWhere(admin, (q) => q.like("source_path", `${prefix}/%`));
    const active = await countWhere(admin, (q) =>
      q.like("source_path", `${prefix}/%`).eq("is_published", true).eq("is_deleted", false),
    );
    const deleted = await countWhere(admin, (q) =>
      q.like("source_path", `${prefix}/%`).eq("is_deleted", true),
    );
