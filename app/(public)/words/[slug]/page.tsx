...
import { getCachedVocabGraphForSlug, type VocabGraphData } from "@/lib/vocab-graph";
...
// Revalidate once per day. The content is markdown-driven and only changes
// after a GitHub import sync, so a 24h TTL is a safe balance between
// freshness and build/load cost.
export const revalidate = 86400;
// Bump from 48 → 500 so the ~500 most-recently-updated words are statically
// generated at build time instead of hitting ISR on first request.
...
export async function WordDetailContent({
  ...
  const [result, graphData] = await Promise.all([
    ...
    getCachedVocabGraphForSlug(slug),
  ]);
  ...
  const safeGraphData = graphData ?? { centerId: slug, nodes: [], edges: [] };
  const relatedReviewWordIds = getRelatedReviewWordIds(safeGraphData);
        ...
        <div className="space-y-6">
          ...
          */}
          <RevealSection delay={0.12}>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <BentoCard
                ...
                preview={`${safeGraphData.nodes.length} 节点 · ${safeGraphData.edges.length} 关联`}
                subtitle="vocab topology"
                title="词汇拓扑图谱"
                variant="accent"
              >
                <VocabTopologyGraphIsland data={safeGraphData} maxNodes={60} />
              </BentoCard>
          ...
          <section className="flex justify-end">
            <a
              href={`/p/${encodeURIComponent(result.word.slug)}`}
