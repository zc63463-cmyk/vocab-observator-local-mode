...
export function VocabTopologyGraph({
  ...
  // Clear navigating state when the route actually changes (new page loads).
  useEffect(() => {
    startTransition(() => {
      setNavigating(false);
    });
  ...
  return (
    <section className={cn("vocab-topology panel rounded-[1.75rem] p-5 sm:p-6", className)}>
      ...
      <div
        ref={containerRef}
        className="vocab-topology__canvas relative mt-4 h-[360px] overflow-hidden rounded-[1.25rem] border border-[var(--color-border)] bg-[var(--color-surface-soft)] md:h-[560px]"
      >
        <svg
          ref={svgRef}
          role="img"
        ...
        />
        {navigating ? (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 rounded-[1.25rem] bg-[var(--color-surface)]/80 backdrop-blur-sm">
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-[var(--color-border-strong)] border-t-[var(--color-accent)]" />
            <p className="text-sm font-medium text-[var(--color-ink-soft)]">正在加载词条…</p>
          </div>
        ) : null}
      </div>
