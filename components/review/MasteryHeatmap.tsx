...
export function MasteryHeatmap({
  ...
  // Refs to diff nodes/edges/dims so we don't tear down and rebuild the d3
  // simulation when the parent re-renders with a new array identity but
  // identical data.
  ...
  useEffect(() => {
    ...
    // Diff against previous render to avoid rebuilding the simulation when
    // the parent passes a new array identity with identical content.
    ...
    const nodesUnchanged =
      ...
      nodes.every(
        (n, i) =>
          n.id === prevNodes[i]?.id &&
          n.retrievability === prevNodes[i]?.retrievability,
      );
    const edgeKey = (e: GraphEdge) =>
      `${String(e.source)}--${String(e.target)}--${e.relation}`;
    const edgesUnchanged =
      edges.length === prevEdges.length &&
      edges.every((e, i) => edgeKey(e) === edgeKey(prevEdges[i]!));
    const dimsUnchanged =
      prevDims.width === dims.width && prevDims.height === dims.height;
    if (nodesUnchanged && edgesUnchanged && dimsUnchanged) {
      return;
    }
