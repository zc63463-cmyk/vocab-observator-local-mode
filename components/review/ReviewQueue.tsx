...
import { Sparkles, SpellCheck2, Feather } from "lucide-react";
...
export function ReviewQueue() {
  ...
  // ── Keyboard shortcuts ──
  // Keep latest handlers in refs so the global keydown listener never has to
  // be torn down and re-attached when items/session/pending change.
  const handleRateRef = useRef(handleRate);
  const handleSkipRef = useRef(handleSkip);
  const handleSuspendRef = useRef(handleSuspend);
  useEffect(() => { handleRateRef.current = handleRate; }, [handleRate]);
  useEffect(() => { handleSkipRef.current = handleSkip; }, [handleSkip]);
  useEffect(() => { handleSuspendRef.current = handleSuspend; }, [handleSuspend]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Free Zen: same immersive UI as zen but with user-selected words
              and zero DB persistence. Placed as a secondary zen route. */}
          <a
            href="/review/free"
          ...
          >
            <Feather className="h-4 w-4 text-[var(--color-accent)]" />
            <span>自由禅意</span>
          </a>
