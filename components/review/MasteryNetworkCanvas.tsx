...
/* Stable Canvas props — r3f shallow-compares objects/arrays by reference,
   so inline literals on every orchestrator rerender (e.g. preview modal
   open/close) can force a full Canvas teardown → WebGL context recreation.
   Hoisting them to module scope eliminates the spurious diff. */
const CANVAS_CAMERA = { position: [0, 0, CAMERA_Z] as [number, number, number], fov: FOV };
const CANVAS_DPR: [number, number] = [1, 2];
const CANVAS_GL = { antialias: true, alpha: true };
const CANVAS_STYLE = { width: "100%", height: "100%", touchAction: "none" as const };
...
export function MasteryNetworkCanvas(props: MasteryNetworkCanvasProps) {
  ...
  return (
    <Canvas camera={CANVAS_CAMERA} dpr={CANVAS_DPR} gl={CANVAS_GL} style={CANVAS_STYLE}>
